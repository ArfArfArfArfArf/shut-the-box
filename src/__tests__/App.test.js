...
test('onSelect called', () => {
  ...
  let roll = state.dieValues[0] + state.dieValues[1];

  const numbercard = wrapper.find({'data-testid': `numbercard${roll}`});
  
  numbercard.simulate('click');
...
test('renders scores in the score history section', () => {
  ...
  expect(wrapper.find({'data-testid': 'App-scores'}).text()).toEqual("733413");
});
